...
from model_hw_with_simple_kernel import SimpleLMDWithKernel
...
def main():
    ...
    parser.add_argument('--output', type=str, default='results_super_simple', help='输出目录')
    ...
    # 选择比原始尺寸稍大的能被8整除的尺寸
    new_h = 448  # 接近450且能被8整除
    new_w = 592  # 接近600且能被8整除
    
    print(f"原始图像尺寸: {h}x{w}")
    print(f"手动选择的新尺寸: {new_h}x{new_w}")
    ...
    print(f"检查能否被8整除: 高度 {img_tensor.shape[2] % 8 == 0}, 宽度 {img_tensor.shape[3] % 8 == 0}")
    
    # 再次检查尺寸
    _, _, adjusted_h, adjusted_w = img_tensor.shape
    assert adjusted_h % 8 == 0, f"高度 {adjusted_h} 不能被8整除"
    assert adjusted_w % 8 == 0, f"宽度 {adjusted_w} 不能被8整除"
