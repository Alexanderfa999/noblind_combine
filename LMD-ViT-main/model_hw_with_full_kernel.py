...
class FullLMDWithKernel(nn.Module):
    """集成模糊核预测的完整LMD-ViT模型"""
    def __init__(self, img_size=256, in_chans=3, dd_in=3,
                 ...
                 downsample=None, upsample=None, shift_flag=True, modulator=False,
                 cross_modulator=False, prune_loc=[0, 0, 1, 1, 1, 1, 1, 0, 0],
                 kernel_pred_K=9, kernel_size=33, **kwargs):
        super().__init__()
        
        # 初始化完整的LMD模型
        from model_hw import Downsample, Upsample
    ...
    def load_lmd_weights(self, checkpoint):
        ...
        # 加载权重
        self.lmd.load_state_dict(new_state_dict, strict=False)
        return self.lmd
        
    def load_kernel_weights(self, kernel_checkpoint):
        """加载模糊核预测网络的预训练权重"""
        # 处理权重名称不匹配的问题
        kernel_state_dict = {}
        ...
        # 加载权重
        self.kernel_prediction.load_state_dict(kernel_state_dict, strict=False)
        return self.kernel_prediction 