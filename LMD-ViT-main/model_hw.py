...
def window_partition(x, win_size, dilation_rate=1):
    ...
    if dilation_rate != 1:
        ...
        # 现在H_pad和W_pad一定是win_size的倍数
        x = x.view(B, H_pad // win_size, win_size, W_pad // win_size, win_size, C)
...
def window_reverse(windows, win_size, H, W, dilation_rate=1):
    ...
    B = int(windows.shape[0] / (H_pad * W_pad / win_size / win_size))
    
    if dilation_rate != 1:
        x = windows.permute(0, 5, 3, 4, 1, 2).contiguous()  # B, C*Wh*Ww, H/Wh*W/Ww
        x = F.fold(x, (H_pad, W_pad), kernel_size=win_size, dilation=dilation_rate, padding=4 * (dilation_rate - 1),
                   stride=win_size)
    else:
        x = windows.view(B, H_pad // win_size, W_pad // win_size, win_size, win_size, -1)
